...
import { Chat, Role, User } from '@prisma/client';
...
export default function StaffChat({
  ...
  const filteredGroup =
    userRole === 'ADVISER'
      ? groups?.filter((group) => group.members.some((m) => m.id === userId))
      : groups;
    ...
    <div className='flex h-full w-full'>
      ...
      {groups && groups.length > 0 && selectedGroup ? (
        <div className='flex flex-col gap-2 pl-4 h-full w-full'>
          <div className='flex-1 border rounded-lg p-4 w-full flex flex-col'>
